...
export function checkInEmailHtml({ guestName, listingTitle, city, state, checkOut, nights, guests, reference, tripsUrl }) {
  const fmt = (d) => new Date(d).toLocaleDateString('en-US', { weekday: 'short', month: 'long', day: 'numeric', year: 'numeric' })
  const safeTitle = (listingTitle || 'your stay').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  const safeGuest = (guestName || 'there').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  return BASE(`
    <div style="text-align:center;margin-bottom:28px">
      <div style="width:56px;height:56px;background:#D1FAE5;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:1.6rem;margin-bottom:14px">🏠</div>
      <h1 style="margin:0 0 8px;font-size:1.2rem;font-weight:800;color:#1A1410">You're checked in, ${safeGuest}!</h1>
      <p style="margin:0;font-size:0.88rem;color:#7A6355;line-height:1.6">
        Enjoy your stay at <strong style="color:#1A1410">${safeTitle}</strong>.
      </p>
    </div>
    ...
    ${BTN(tripsUrl, 'View trip details')}
    ${FOOTER_NOTE('Enjoy your stay! Questions? Contact support@snapreserve.app.')}
  `)
}

export function checkInEmailText({ guestName, listingTitle, city, state, checkOut, nights, guests, reference, tripsUrl }) {
  const fmt = (d) => new Date(d).toLocaleDateString('en-US', { weekday: 'short', month: 'long', day: 'numeric', year: 'numeric' })
  return [
    `Hi ${guestName || 'there'}, you're checked in!`,
    '',
    `Property: ${listingTitle}`,
    `Location: ${city}${city && state ? ', ' : ''}${state}`,
    `Check-out: ${fmt(checkOut)}`,
    ...
    `Reference: ${reference}`,
    '',
    `View your trip: ${tripsUrl}`,
    '',
    'Enjoy your stay! Questions? Contact support@snapreserve.app',
  ].join('\n')
}
