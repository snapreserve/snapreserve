...
async function getHosts() {
  ...
  const { data } = await supabase
    .from('hosts')
    .select('id, user_id, verification_status, suspended_at, suspension_reason, created_at, is_snap_verified, snap_verified_at, users(id, email, full_name, is_active), listings(id, type, is_active, status, city, state, rating, review_count, price_per_night)')
